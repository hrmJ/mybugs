...
#jagsModel <- jags.model("model.bugs", data=dataList, n.chains = 1, n.adapt = 50000)

RunJagsModel <- run.jags(data=dataList, monitor=monitor, model="model.bugs",
                            n.chains=2, adapt=50000, burnin=20000, thin=1,
                            method="parallel", sample=30000)

saveRDS(RunJagsModel,"c:Users/jh89963.STAFF/Dropbox/simple3LOC_runjags.rds")
print("SAVED.")

#update(jagsModel, n.iter=5000)
#saveRDS(jagsModel,"adapted_60000_withclausestatus.rds")

saveRDS(RunJagsModel,"simple3LOC_runjags.rds")
#post2 <- coda.samples(jagsModel, variable.names=monitor, n.iter=10000, thin=1)
#saveRDS(post,"10000samples_from_60000_withclausestatus.rds")
